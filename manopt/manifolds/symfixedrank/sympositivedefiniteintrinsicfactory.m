function M = sympositivedefiniteintrinsicfactory(n)
...
%       speed up line-searches.
    ...
    M.name = @() sprintf('Symmetric positive definite intrinsic geometry of %dx%d matrices', n, n);
    ...
    function Y = retraction(X, eta, t)
        ...
        X1half = X^(1/2);
        Xm1half = X^(-1/2);
        Y = symm(X1half*expm(Xm1half*teta*Xm1half)*X1half);
    end
    ...
    % vector transport from Jeuris, Vandebril, Vandereycken.
    M.transp = @vectransJVV;
    function transported = vectransJVV(X, Y, xi)
        eta = M.log(X,Y);
        X1half = X^(1/2);
        Xm1half = X^(-1/2);
        exphalf = expm(Xm1half*eta*Xm1half/2);
        transported = X1half*exphalf * Xm1half*xi*Xm1half * exphalf*X1half;
    end
